...
class _UnifiedCharacterEditorScreenState extends State<UnifiedCharacterEditorScreen> 
  ...
  void _initializeComponents() {
    ...
    // Inventory Handler wird vorerst nicht verwendet, da er mit EnhancedController nicht kompatibel ist
    ...
    _tabManager = CharacterTabManager(
      ...
      formKey: _formKey,
      inventoryHandler: null, // vorerst deaktiviert
    );
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Fehler beim Laden des Inventars: $e'),
  ...
  Future<void> _importFromOfficialMonster() async {
    ...
    if (selectedMonster != null && selectedMonster is OfficialMonster && mounted) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('${selectedMonster.name} wurde importiert'),
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (context) => CharacterEditorViewModel(),
      child: Scaffold(
