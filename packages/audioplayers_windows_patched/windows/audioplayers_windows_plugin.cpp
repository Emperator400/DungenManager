...
//
// Media Foundation callbacks fire on MF worker threads. Sending Flutter
// platform-channel messages from those threads triggers:
//   "channel sent a message … on a non-platform thread"
// and can cause dropped events or crashes in debug builds.
//
// Fix: queue callbacks and dispatch them to the Flutter platform thread via
// PostMessage. RegisterWithRegistrar hooks a TopLevelWindowProcDelegate that
// drains the queue when WM_AUDIO_PLATFORM_DISPATCH arrives.
// ---------------------------------------------------------------------------
namespace {
static HWND g_flutter_hwnd = nullptr;
static std::mutex g_dispatch_mutex;
static std::queue<std::function<void()>> g_dispatch_queue;
static constexpr UINT WM_AUDIO_PLATFORM_DISPATCH = WM_APP + 42;
}  // namespace

// Posts |callback| for execution on the Flutter platform thread.
// Safe to call from any thread.
static void PostToFlutterThread(std::function<void()> callback) {
  if (!g_flutter_hwnd) {
    callback();  // fallback (should not happen after init)
    return;
  }
  {
    std::lock_guard<std::mutex> lock(g_dispatch_mutex);
    g_dispatch_queue.push(std::move(callback));
  }
  PostMessage(g_flutter_hwnd, WM_AUDIO_PLATFORM_DISPATCH, 0, 0);
}
...
void AudioplayersWindowsPlugin::RegisterWithRegistrar(
  ...
  // Store the Flutter window handle so PostToFlutterThread can dispatch back.
  g_flutter_hwnd = registrar->GetView();

  // Register a WndProc delegate to drain the dispatch queue on the platform
  // thread when WM_AUDIO_PLATFORM_DISPATCH arrives.
  registrar->RegisterTopLevelWindowProcDelegate(
      [](HWND hwnd, UINT msg, WPARAM, LPARAM) -> std::optional<LRESULT> {
        if (msg == WM_AUDIO_PLATFORM_DISPATCH) {
          // Drain all queued callbacks on the platform thread.
          while (true) {
            std::function<void()> cb;
            {
              std::lock_guard<std::mutex> lock(g_dispatch_mutex);
              if (g_dispatch_queue.empty()) break;
              cb = std::move(g_dispatch_queue.front());
              g_dispatch_queue.pop();
            }
            cb();
          }
          return 0L;
